...
void test_predicate_macros()
{
    int even_number = 4;
    int odd_number = 3;
    assert(!(even odd_number));
    assert(odd odd_number);
    assert(!(odd even_number));
    assert(positive 5);
    assert(!(positive - 1));
    assert(negative - 5);
    assert(!(negative 3));
