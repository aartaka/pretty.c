...
*** ~foriter (type, var, iter, ...)~
Iterates over vararg data using the ~iter~ single-parameter
function. If ~iter~ returns ~nil~, end. If it returns new value,
assign ~type~-d ~var~ to it.
